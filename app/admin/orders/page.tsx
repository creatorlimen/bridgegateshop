  ...
  return (
    <section>
      ...
      <p className="mt-5 max-w-2xl text-sm leading-6 text-muted">
        Open an order to approve eligible POD, verify transfers, reconcile
        collections, review refunds, or apply guarded cancellation.
      </p>
      <div className="mt-8 overflow-x-auto rounded-2xl border border-ink/10 bg-paper">
            ...
            {orders.map((order) => (
              <tr className="border-b border-ink/10" key={order.id}>
                ...
                <td className="p-4 text-right font-black">
                  {formatMoney(order.totals.grandTotalKobo)}
                </td>
              </tr>
            ))}
          </tbody>
        ...
        {!orders.length ? (
          <p className="p-6 text-sm text-muted">No orders have been placed.</p>
        ) : null}
      </div>
    </section>
  );
