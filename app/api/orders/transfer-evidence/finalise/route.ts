...
import { finaliseTransferEvidenceInputSchema } from '@/lib/schemas/transferEvidenceUpload';
...
export async function POST(request: Request) {
  ...
  const parsed = finaliseTransferEvidenceInputSchema.safeParse(await request.json());
  if (!parsed.success) return NextResponse.json({ ok: false, message: 'Evidence finalisation request is invalid.', requestId }, { status: 400 });
  try {
    const settings = await loadCheckoutSettings();
    const data = await createTransferEvidenceService(undefined, undefined, settings).finalise(parsed.data, proof);
    ...
    return NextResponse.json({ ok: false, message: error instanceof TransferEvidenceError ? error.message : 'Evidence upload could not finish.', requestId }, { status });
  }
}
